...
def normalize(raw_user_input: str) -> dict:
    ...
    user_command: dict = {'command': None, 'name': None, 'phone': None, 'new_phone': None}
    ...
    if user_input in ['hello', 'show all', 'good buy', 'close', 'exit']:
        ...
        if len(user_input_list) > 2: user_command['phone'] = user_input_list[2]
        if len(user_input_list) > 3: user_command['new_phone'] = user_input_list[3]
